		...
		},
		{
			...
			pkg: &config.Package{
				...
				PackageInfo: &registry.PackageInfo{
					...
					RepoOwner: "hashicorp",
					RepoName:  "terrafrom",
					URL:       ptr.StrP("https://releases.hashicorp.com/terraform/{{trimV .Version}}/terraform_{{trimV .Version}}_{{.OS}}_{{.Arch}}.zip"),
				},
			},
			rt: &runtime.Runtime{
				GOOS:   "darwin",
	...
	}{
		{
			...
			pkg: &config.Package{
				...
				PackageInfo: &registry.PackageInfo{
					...
					RepoOwner: "helm",
					RepoName:  "helm",
					URL:       ptr.StrP("https://get.helm.sh/helm-{{.Version}}-{{.OS}}-{{.Arch}}.tar.gz"),
					Checksum: &registry.Checksum{
						Type: "http",
