...
func (c *Controller) GenerateRegistry(ctx context.Context, param *config.Param, logE *logrus.Entry, args ...string) error {
	...
	if param.GenerateConfigFilePath != "" {
		err := readConfig(c.fs, param.GenerateConfigFilePath, cfg)
		if err != nil {
			return err
		}
	}
	...
	for _, arg := range args {
		err := c.genRegistry(ctx, param, logE, cfg, arg)
		if err != nil {
			return err
		}
	}
...
func (c *Controller) genRegistry(ctx context.Context, param *config.Param, logE *logrus.Entry, cfg *Config, pkgName string) error {
	...
	if param.OutTestData != "" {
		err := c.testdataOutputter.Output(&output.Param{
	...
	if param.InsertFile == "" {
		...
		err := encoder.EncodeContext(ctx, cfg)
		if err != nil {
	...
	err := c.insert(param.InsertFile, registry.PackageInfos{pkgInfo})
	if err != nil {
		return err
	}
