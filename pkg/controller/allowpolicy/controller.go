package allowpolicy

import (
	"fmt"
	"log/slog"

	"github.com/aquaproj/aqua/v2/pkg/config"
	"github.com/aquaproj/aqua/v2/pkg/policy"
	"github.com/suzuki-shunsuke/slog-error/slogerr"
)

type Controller struct {
	policyConfigFinder policy.ConfigFinder
	policyValidator    policy.Validator
}

func New(policyConfigFinder policy.ConfigFinder, policyValidator policy.Validator) *Controller {
	return &Controller{
		policyConfigFinder: policyConfigFinder,
		policyValidator:    policyValidator,
	}
}

func (c *Controller) Allow(logger *slog.Logger, param *config.Param, policyFilePath string) error {
	policyFile, err := c.policyConfigFinder.Find(policyFilePath, param.CWD)
	if err != nil {
		return fmt.Errorf("find a policy file: %w", err)
	}
	if policyFile == "" {
		logger.Info("no policy file is found")
		return nil
	}
	if err := c.policyValidator.Allow(policyFile); err != nil {
		return slogerr.With(fmt.Errorf("allow a policy file: %w", err), //nolint:wrapcheck
			"policy_file", policyFile,
		)
	}
	return nil
}
