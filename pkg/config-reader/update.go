...
func (r *ConfigReader) readImportsToUpdate(configFilePath string, cfg *aqua.Config) (map[string]*aqua.Config, error) { //nolint:cyclop
	...
	for _, pkg := range cfg.Packages {
		...
		err := r.readImportToUpdate(configFilePath, pkg.Import, cfg, cfgs)
		if err != nil {
			return nil, err
		}
	}
	...
	if cfg.ImportDir != "" {
		err := r.readImportToUpdate(configFilePath, filepath.Join(cfg.ImportDir, "*.yml"), cfg, cfgs)
		if err != nil {
			return nil, err
		}
		err := r.readImportToUpdate(configFilePath, filepath.Join(cfg.ImportDir, "*.yaml"), cfg, cfgs)
		if err != nil {
			return nil, err
		}
	}
